...
function displayExp(details) {
  ...
  for (i = 0; i < details.length; i++) {
    ...
    <div class="expValue" id="${details[i].id}">
      ...
      <div id="edite_delete">
        <p>
          <button id="${details[i].id}" onclick="editExpDetails(${details[i].id})"><img src="image/edit.jpg.png" width="30" alt=""  /></button> 
          <button id="${details[i].id}" onclick="delExpenseDetails(${details[i].id})"><img src="image/delete.jpg.png" width="30" alt="" /></button>
        </p>
      </div>
    </div>
  `;
